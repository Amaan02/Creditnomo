...
export const createHistorySlice: StateCreator<HistoryState> = (set, get) => ({
  ...
  fetchHistory: async (playerAddress: string) => {
    try {
      ...
      try {
        // Fetch a larger window so stats (net profit, win rate) are accurate
        const res = await fetch(`/api/bets/history?wallet=${encodeURIComponent(playerAddress)}&limit=500`);
        if (res.ok) {
  ...
  clearHistory: () => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.removeItem('creditnomo_bet_history');
    }
  },
...
const saveBetsToLocalStorage = (bets: BetRecord[]): void => {
  try {
    if (typeof window !== 'undefined') {
      localStorage.setItem('creditnomo_bet_history', JSON.stringify(bets));
    }
  } catch (error) {
...
const loadBetsFromLocalStorage = (): BetRecord[] => {
  try {
    if (typeof window !== 'undefined') {
      const data = localStorage.getItem('creditnomo_bet_history');
      if (data) {

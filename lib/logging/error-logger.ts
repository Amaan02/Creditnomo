...
function getUserAddress(): string | undefined {
  ...
  try {
    ...
    const sessionData = localStorage.getItem('creditnomo_wallet_session');
    if (sessionData) {
...
function getSessionId(): string {
  ...
  try {
    let sessionId = sessionStorage.getItem('creditnomo_session_id');
    if (!sessionId) {
...
function storeErrorLocally(errorLog: ErrorLogEntry): void {
  ...
  try {
    const key = 'creditnomo_error_logs';
...
export function clearStoredErrorLogs(): void {
  ...
  try {
    localStorage.removeItem('creditnomo_error_logs');

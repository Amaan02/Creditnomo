...
export const BalanceDisplay: React.FC = () => {
  const houseBalance = useCreditnomoStore(state => state.houseBalance);
  const demoBalance = useCreditnomoStore(state => state.demoBalance);
  const accountType = useCreditnomoStore(state => state.accountType);
  const network = useCreditnomoStore(state => state.network);
  const toggleAccountType = useCreditnomoStore(state => state.toggleAccountType);
  const isLoading = useCreditnomoStore(state => state.isLoading);
  const address = useCreditnomoStore(state => state.address);
  ...
  const setAddress = useCreditnomoStore(state => state.setAddress);
  const setIsConnected = useCreditnomoStore(state => state.setIsConnected);
  ...
  return (
    <>
      <div className="bg-black/30 rounded-xl border border-white/5 overflow-hidden">
        {/* Secret Demo Header - Only visible when activated via CREDITNOMO logo click */}
        {accountType === 'demo' && (

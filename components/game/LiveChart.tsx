...
const AssetIcon = ({ src, asset, className }: { src: string; asset: string; className: string }) => {
  ...
  return (
    <div className={`relative flex items-center justify-center overflow-hidden w-full h-full ${isMetal ? 'rounded-full border border-yellow-400/50 shadow-[0_0_10px_rgba(234,179,8,0.3)] bg-gradient-to-br from-yellow-400/20 to-black' : ''} ${isForex ? 'rounded-full border border-white/20 bg-black/40' : ''}`}>
      <img
        src={src}
        alt={asset}
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-[#02040A]/80 backdrop-blur-sm">
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-gray-500 text-xs">Connecting to market price feeds...</p>
            </div>
          </div>
        </div>
      )}
          ...
          {betCells.map((cell: any) => {
            ...
            const handleClick = async () => {
              if (canBet && betAmount && userAddress) {
                ...
                const currentBalance = accountType === 'demo' ? demoBalance : houseBalance;
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-1.5 mt-1 opacity-50">
              ...
              <span className="text-[9px] text-gray-400 font-black tracking-[0.2em] uppercase">
                Powered by CoinGecko · DexScreener · CMC
              </span>
            </div>
          </div>
        </div>

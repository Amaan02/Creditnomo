...
export const WalletConnect: React.FC = () => {
  ...
  const { network, address, setConnectModalOpen, disconnect: disconnectStore, setPreferredNetwork, setAddress, setIsConnected } = useCreditnomoStore();
  ...
  useEffect(() => {
    const fetchBalance = async () => {
      ...
      try {
        ...
        useCreditnomoStore.setState({ walletBalance: isNaN(balNum) ? 0 : balNum });
      } catch (error) {
